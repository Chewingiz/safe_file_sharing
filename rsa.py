...
def rsa (msg, key):
   ...
   # print(n)
   # print(msg)
    assert(msg<n)#chat gpt m'a dit de tester sans 
    c= pow(msg,e,n)
    return c

def rsa_enc (msg,key):
	#return rsa(int.from_bytes(msg, 'big'), key)
	return rsa(int.from_bytes(msg.encode('utf-8'), 'big'), key)
...
def rsa_dec(enc_msg,key):
	...
	#msg=msg.to_bytes((msg.bit_length() + 7) // 8, 'big').decode('utf-8')
	#msg.to_bytes((msg.bit_length() + 7) // 8, 'big')
	msg = msg.to_bytes((msg.bit_length() + 7) // 8, 'big')#msg.to_bytes((msg.bit_length() + 7) // 8, 'big').decode('utf-8')
	return msg
"""
"""
def rsa_dec(enc_msg,key):
